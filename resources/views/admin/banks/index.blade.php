...
<div class="container">
    <div class="row">
	    ...
	    <table class="bordered striped highlight centered responsive-table z-pedth-5">
	    	<thead>
	    		<tr>
	    			<th>#</th>
	    			<th>Nome</th>
		    		...
		    		<tr>
		    			...
		    			<td>
		    				...
		    				<delete-action action="{{route('admin.banks.destroy',['bank' => $bank->id])}}" 
				                ...
				                <?php $modalId = "model-delete-$bank->id";?>
			    				...
			    				<modal :modal="{{json_encode(['id'=> $modalId])}}" style="display: none">
			    					<div slot="content">
			    					    ...
			    					    <p><strong>Deseja esxcluir este banco</strong></p>
			    					    <div class="divider"></div>
			    					    <p>Nome: <strong>{{$bank->name}}</strong></p>
			    					    <div class="divider"></div>
			    					</div>
			    					<div slot="footer">
			    						<button class="btn btn-flat waves-effect green lighten-2 modal-action"
			    						        id="link-delete-{{$bank->id}}">OK</button>
			    						<button class="btn btn-flat waves-effect wave-red modal-close modal-action">Cancelar</button>
			    					</div>
			    				</modal>
		    				</delete-action>
